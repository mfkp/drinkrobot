class IngredientsRecipesController < ApplicationController
  ...
  def index
    @ingredients_recipes = IngredientsRecipes.find(:all)
  ...
  def create
    ...
    respond_to do |format|
      if @ingredients_recipes.save
        flash[:notice] = 'IngredientsRecipes was successfully created.'
        format.html { redirect_to(@ingredients_recipes) }
  ...
  def update
    ...
    respond_to do |format|
      if @ingredients_recipes.update_attributes(params[:ingredients_recipes])
        flash[:notice] = 'IngredientsRecipes was successfully updated.'
        format.html { redirect_to(@ingredients_recipes) }
        format.xml  { head :ok }
      else
